...
export class EstudianteComponent implements OnInit {
  ...
  deleteCupo(matricula: Participante) {
    swal.fire(this.messages['deleteQuestion'])
      .then((result) => {
        if (result.value) {
          this.spinner.show();
    ...
    if (event.which === 13 || this.buscador.length === 0) {
      if (this.buscador.length === 0) {
        ...
        this.service.get('matriculas/cupo' + parametros).subscribe(
          response => {
            this.participantes = response['cupo'];
            this.spinner.hide();
            this.total_register = this.participantes.length;
          },
          error => {
            this.spinner.hide();
            swal.fire(this.messages['error500']);
  ...
  getCupos(page: number) {
    ...
    this.service.post('estudiantes/en_proceso' + parametros, null).subscribe(
      response => {
        this.participantes = response['participantes']['data'];
  ...
  uploadEstudiantes(ev) {
    ...
    if (this.archivo.files.length > 0) {
      ...
      this.service.upload('imports/participantes', form).subscribe(
        response => {
          this.getCupos(1);
          this.spinner.hide();
  ...
  validateCupo(cupo: Participante) {
        ...
        if (result.value) {
          ...
          this.service.delete('matriculas/delete_estudiantes_carrera?carrera_id=' + this.carrera.id).subscribe(
            response => {
              ...
              this.spinner.hide();
              this.total_register = this.participantes.length;
              this.messages['deleteSuccess']['text'] = response['participantes'] + ' ' + this.messages['deleteSuccess']['text'];
              swal.fire(this.messages['deleteSuccess']);
            },
            error => {
              this.spinner.hide();
